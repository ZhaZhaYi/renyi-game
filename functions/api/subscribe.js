...
async function sendConfirmationEmail(env, email, token, origin, gameName) {
    const from = env.RESEND_FROM || 'RenyiGame <noreply@mail.renyigame.com>';
    const confirmUrl = origin + '/api/confirm?token=' + encodeURIComponent(token);
    const subject = '确认订阅 · RenyiGame · ' + gameName;
    const html =
        ...
        '  <p style="text-align:center;color:#a08a76;font-size:13px;margin-bottom:22px">RenyiGame · ' + gameName + ' 新作与内测消息第一时间通知你</p>' +
        ...
        '  </div>' +
        '  <p style="text-align:center;color:#c9b8a5;font-size:12px;margin-top:22px">© 2026 RenyiGame · 适龄游戏，理性娱乐</p>' +
        '</div>';
...
export async function onRequestPost(context) {
    ...
    try {
        ...
        try {
            ...
            gameName = (body && body.game) || env.GAME_NAME || 'TileBuddies';
        } catch (e) {
        ...
        email = email.trim();
        gameName = (typeof gameName === 'string' && gameName.trim()) || 'TileBuddies';

        const origin = new URL(request.url).origin;

        // 3. 查重：已确认 -> 提示；待确认 -> 重发确认邮件
        const existing = await env.DB
            .prepare("SELECT id, status FROM subscribers WHERE email = ? COLLATE NOCASE AND game = ? LIMIT 1")
            .bind(email, gameName)
            .first();

        if (existing) {
            if (existing.status === 'confirmed') {
                return json({ success: false, message: '这个邮箱已经订阅过《' + gameName + '》了哦！' }, 409);
            }
            ...
            await env.DB
                ...
                .run();
            try {
                await sendConfirmationEmail(env, email, token, origin, gameName);
            } catch (sendErr) {
                console.error('Resend error:', sendErr);
                return json({ success: false, message: '确认邮件发送失败，请稍后再试。' }, 502);
            }
            return json({ success: true, message: '《' + gameName + '》的确认邮件已重新发送，请到邮箱点击链接完成订阅。' }, 200);
        }

        // 4. 新订阅：写入待确认状态
        const token = crypto.randomUUID();
        await env.DB
            .prepare("INSERT INTO subscribers (email, status, token, game) VALUES (?, 'pending', ?, ?)")
            .bind(email, token, gameName)
            .run();
        ...
        try {
            ...
            console.error('Resend error:', sendErr);
            await env.DB
                .prepare('DELETE FROM subscribers WHERE email = ? COLLATE NOCASE AND game = ?')
                .bind(email, gameName)
                .run()
                .catch(() => {});
            return json({ success: false, message: '确认邮件发送失败，请稍后再试。' }, 502);
